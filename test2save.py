...
checkpoint=torch.load(os.path.join(model_dir,'model_latest.pth'),map_location=torch.device('cpu'))
...
_,test_loader,_=create_dataloaders(train='/home/huangweiyan/workspace/model/cv/data/LOLv1/Train',
...
for idx, data in enumerate(tqdm(test_loader, desc="save images")):
    with torch.no_grad():
        ...
        image_pil = to_pil(restored[2].squeeze(0))
        
        # 保存图像
        image_pil.save(os.path.join(savedir, 'small'+name))

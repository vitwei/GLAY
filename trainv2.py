...
def cfg():
    ...
    parser.add_argument('--modelname', default='newgll', help='define model name')
...
def eval(model, epoch, writer, txt_write, opt):
    ...
    test_LL_folder = os.path.join(opt.testset, "input")
    test_NL_folder = os.path.join(opt.testset, "target")
...
def main(opt):
    ...
    with open(txt_name, mode='w') as txt_write:
        ...
        print(network_parameters(lighten))
        print('----------------------------------------------')
        ...
        for epoch in range(opt.start_iter, opt.nEpochs + 1):
            ...
            if epoch % (opt.snapshots) and epoch>100 == 0:
